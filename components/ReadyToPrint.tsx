  ...
  if (isPrinting) {
    return (
      <div className="bg-white min-h-screen relative">
         ...
         <div className="no-print fixed top-0 left-0 right-0 bg-white shadow-lg z-50 p-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
             ...
             <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
                 <button 
                    onClick={handleBack}
                    className="flex-1 md:flex-none px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg font-medium border border-gray-300 whitespace-nowrap"
                 >
                     Batal
                 </button>
                 <button 
                    onClick={() => window.print()}
                    className="flex-1 md:flex-none px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-lg font-bold flex items-center justify-center gap-2 whitespace-nowrap"
                 >
                     <i className="fa-solid fa-print"></i> Cetak
                 </button>
                 <button 
                    onClick={handleFinishPrinting}
                    className="flex-1 md:flex-none px-4 py-2 bg-green-600 text-white hover:bg-green-700 rounded-lg font-bold flex items-center justify-center gap-2 whitespace-nowrap"
                 >
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 p-4 pt-40 md:pt-24 print:p-0 print:pt-0 print:gap-x-4 print:gap-y-2">
            {packages.filter(p => selectedIds.has(p.id)).map(pkg => (
            <div 
                key={pkg.id} 
                className="border-2 border-black rounded-lg relative flex flex-col justify-between overflow-hidden bg-white mx-auto w-full max-w-[400px] md:max-w-none print:max-w-none" 
                style={{ height: '7cm' }}
            >
                {/* Header */}
  ...
  return (
    <div className="space-y-6 relative pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 gap-4">
        <div>
           <h2 className="text-xl md:text-2xl font-bold text-gray-800">Antrian Cetak & Kirim</h2>
           <p className="text-sm text-gray-500">Pilih paket untuk dicetak, atau input resi.</p>
        </div>
        <div className="flex w-full md:w-auto gap-3">
             <button
                onClick={loadPackages}
                className="p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg transition border border-gray-200"
                title="Refresh"
             >
            ...
            <button
                ...
                className="flex-1 md:flex-none bg-secondary text-white px-6 py-2.5 rounded-lg font-bold hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-sm"
            >
        ...
        <>
            ...
            <div className="hidden md:block overflow-x-auto">
                ...
                <tbody className="divide-y">
                    ...
                    >
                        <td className="p-4 text-center cursor-pointer" onClick={(e) => { e.stopPropagation(); toggleSelect(pkg.id); }}>
                        <input
                            type="checkbox"
            ...
            <div className="md:hidden bg-gray-50 p-4 space-y-4">
                <div className="flex items-center gap-3 mb-2 px-1">
                    <input
                        ...
                        id="selectAllMobile"
                    />
                    <label htmlFor="selectAllMobile" className="text-sm font-bold text-gray-700">Pilih Semua ({packages.length})</label>
                </div>
                ...
                {packages.map((pkg) => (
                    <div 
                        ...
                        className={`bg-white p-4 rounded-xl border transition-all ${selectedIds.has(pkg.id) ? 'border-primary ring-1 ring-primary shadow-md' : 'border-gray-200 shadow-sm'}`}
                    >
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-gray-800">{pkg.recipientName}</h3>
                                    <p className="text-xs text-gray-500">{pkg.phoneNumber}</p>
                                </div>
                            </div>
                            <span className="px-2 py-1 bg-gray-100 rounded text-xs font-bold text-gray-600">
                                {pkg.courier}
                        ...
                        <div className="pl-8 mb-4">
                            <p className="text-sm text-gray-600 line-clamp-2 bg-gray-50 p-2 rounded border border-gray-100 mb-2">
                                {pkg.address}, {pkg.district}, {pkg.city}
                            </p>
                            {pkg.status === ShippingStatus.PRINTED && (
                                <span className="inline-block px-2 py-0.5 rounded text-[10px] bg-blue-100 text-blue-800 font-bold">
                                    Sudah Dicetak
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex gap-2 items-center pl-8">
                             <input 
                                ...
                                value={resiInputs[pkg.id] || pkg.shippingCode || ''}
                                onChange={(e) => handleResiChange(pkg.id, e.target.value)}
                                className="flex-1 bg-white border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                                onClick={(e) => e.stopPropagation()} 
                            />
                            <button
                                onClick={(e) => { e.stopPropagation(); submitResi(pkg.id); }}
                                className="bg-green-600 hover:bg-green-700 text-white w-10 h-10 rounded-lg flex items-center justify-center transition-colors shadow-sm"
                                title="Simpan & Kirim"
                            >
                             ...
                             <button
                                onClick={(e) => handleEditClick(pkg, e)}
                                className="bg-gray-100 hover:bg-blue-100 text-gray-600 hover:text-blue-600 w-10 h-10 rounded-lg flex items-center justify-center transition-colors border border-gray-200"
                                title="Edit Alamat"
                            >
      ...
      {editingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="p-4 md:p-5 border-t border-gray-100 flex justify-end gap-2 bg-white">
                    <button 
